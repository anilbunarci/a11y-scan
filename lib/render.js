  ...
  const rowHtml = deviceRows
    ...
    <tr>
      ...
      <td>
        ...
        ${row.pa11yReportHtml ? ` ${row.reportHtml ? "<br>" : ""} <a href="./${esc(row.pa11yReportHtml)}">Pa11y</a>` : ""}
      </td>
    ...
    <table class="fixed">
      ...
      <thead>
        <tr>
          ...
          <th title="Accessibility">A11y</th>
          <th title="Best Practices">Best Pcs</th>
          <th>SEO</th>
          <th>Issue Count</th>
          <th>A11y Findings</th>
          <th>Extended A11y</th>
...
function renderHtmlDashboard(summary, rows, generatedAt, sitemapUrl) {
  ...
  const tabButtons = devices
    .map(
      (d) =>
        `<button class="tab-btn${d === firstDevice ? " active" : ""}" data-device="${esc(d)}" onclick="switchTab('${esc(d)}',this)">${tabIcons[d] ?? ""}${d.charAt(0).toUpperCase() + d.slice(1)}</button>`,
    )
    .join("");
...
<body>
  ...
  <script>
    ...
    document.querySelectorAll('.pg-bar').forEach(function(bar) {
      ...
      sel.addEventListener('change', function() { perPage = +sel.value; page = 1; render(); });
      render();
    });
  </script>
</body>
