...
function formatDate(dateStr) {
  ...
  // Shift to EDT (fixed UTC−4 offset)
  ...
  let hours = est.getUTCHours();
  const mins = String(est.getUTCMinutes()).padStart(2, "0");
  const ampm = hours >= 12 ? "PM" : "AM";
  hours = hours % 12 || 12;
  return `${day}/${month}/${year} - ${hours}:${mins} ${ampm} EDT`;
}
...
function healthKey(avg) {
  if (avg === null || avg === undefined) return "failed";
  if (avg >= SCORE_PASS) return "pass";
  if (avg >= SCORE_AVG) return "needs-attention";
  return "critical";
}

const filterIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3"/></svg>`;

const CATEGORY_LABELS = {
  accessibility: "Accessibility",
  "accessibility-extended": "Accessibility Extended",
...
function buildFilterPopover(popoverId, sections, tbodyId) {
  const sectionsHtml = sections
    .map(({ title, filterKey, options }) => {
      const optsHtml = options
        .map(
          ([label, value, count]) => `
      <div class="popover-opt">
        <label><input type="checkbox" class="filter-check" data-table="${tbodyId}" data-filter-key="${filterKey}" value="${esc(value)}"> ${esc(label)}</label>
        <span class="popover-count">${count}</span>
      </div>`,
        )
        .join("");
      return `<div class="popover-section"><div class="popover-section-hdr"><span class="popover-section-title">${esc(title)}</span><button class="popover-reset" data-table="${tbodyId}" data-filter-key="${filterKey}">Reset</button></div><div class="popover-options">${optsHtml}</div></div>`;
    })
    .join("");
  return `<div class="popover-wrap"><button class="filter-btn" data-popover="${popoverId}" aria-label="Filter">${filterIcon}</button><div class="popover" id="${popoverId}">${sectionsHtml}</div></div>`;
}
...
function buildNextStepsHtml(deviceRows, commonIssues) {
  const successful = deviceRows.filter((r) => r.status === "success");
  ...
  const catScores = [
    { cat: "Performance", score: avg("performance") },
    { cat: "Accessibility", score: avg("accessibility") },
    { cat: "Best Practices", score: avg("bestPractices") },
    { cat: "SEO", score: avg("seo") },
  ]
    .filter((c) => c.score !== null)
    .sort((a, b) => a.score - b.score);

  const hasA11yIssues = commonIssues.some((i) =>
    (i.category || "").toLowerCase().includes("accessibility"),
  );

  const stepText = {
    Performance: (score) =>
      `<strong>Improve Page Performance</strong> <span class="next-step-score">(avg. ${score}/100)</span> — Reduce load times by optimizing images, deferring unused JavaScript and CSS. Start with the pages listed in the Worst Performing Pages table.`,
    Accessibility: (score) =>
      `<strong>Remediate Accessibility Issues</strong> <span class="next-step-score">(avg. ${score}/100)</span> — Fix colour-contrast failures, missing or incorrect ARIA attributes, and keyboard-navigation barriers. Prioritize issues that appear across the most pages and those with the highest occurrence counts in the Most Common Issues table.`,
    "Best Practices": (score) =>
      `<strong>Resolve Best Practices Violations</strong> <span class="next-step-score">(avg. ${score}/100)</span> — Replace legacy code patterns, update outdated third-party libraries, and ensure all assets are served securely. These changes reduce security risk and improve long-term maintainability.`,
    SEO: (score) =>
      `<strong>Strengthen SEO Foundations</strong> <span class="next-step-score">(avg. ${score}/100)</span> — Verify that every page has a unique title, meta description, valid structured data, and a fully crawlable link structure to support search visibility.`,
  };

  const steps = [];

  for (const { cat, score } of catScores) {
    if (score < SCORE_PASS && stepText[cat]) {
      steps.push(stepText[cat](score));
    }
  }

  // If accessibility issues dominate but Accessibility score is passing, still surface the step
  if (hasA11yIssues && !steps.some((s) => s.includes("Accessibility"))) {
    const aScore =
      catScores.find((c) => c.cat === "Accessibility")?.score ?? "—";
    steps.unshift(stepText["Accessibility"](aScore));
  }

  steps.push(
    `<strong>Establish an Ongoing Review Cadence</strong> — Schedule re-scans after every major release and at regular intervals to catch regressions early. Platforms such as Siteimprove or Monsido can automate monitoring and provide compliance dashboards for continued oversight.`,
  );

  const listItems = steps
    .map(
      (text, i) =>
        `<li class="next-step-item"><span class="next-step-num">${i + 1}</span><span class="next-step-text">${text}</span></li>`,
    )
    .join("");
  ...
  <div class="next-steps">
    ...
    <p class="next-steps-intro">Addressing the following items will deliver improvement to overall site health:</p>
    <ol class="next-steps-list">${listItems}</ol>
  </div>`;
}
...
function renderTabPanel(device, rows) {
  ...
  const bestRows = getBestRows(deviceRows, 10);
  ...
  const categoryFilterHtml = buildFilterPopover(
    `cat-filter-${device}`,
    [
      {
        title: "Category",
        ...
        options: Object.entries(categoryCounts)
          ...
          .map(([cat, count]) => [formatCategoryLabel(cat), cat, count]),
      },
    ],
    `issues-tbody-${device}`,
  );
  ...
  for (const row of deviceRows) {
    healthCounts[healthKey(scoreAverageForRow(row))]++;
  }
  const healthFilterHtml = buildFilterPopover(
    `health-filter-${device}`,
    [
      {
        title: "Health",
        filterKey: "health",
        options: [
          ["Pass", "pass", healthCounts.pass],
          [
            "Needs Attention",
            "needs-attention",
            healthCounts["needs-attention"],
          ],
          ["Critical", "critical", healthCounts.critical],
          ["Failed", "failed", healthCounts.failed],
        ],
      },
    ],
    `results-tbody-${device}`,
  );

  const issueListHtml = commonIssues.length
    ? commonIssues
        .map((issue) => {
          // Lookup order: curated map → inline "Recommendation:" in title → prose description → empty
          const curated = getRecommendation(issue.id);
          let rec = "";
          if (curated) {
            rec = esc(curated);
          } else {
            // pa11y messages often embed "Recommendation: ..." inline in the title
            const inlineMatch = (issue.title || "").match(
              /\bRecommendation:\s*(.+)/i,
            );
            if (inlineMatch) {
              const t = inlineMatch[1].trim();
              rec = esc(t.charAt(0).toUpperCase() + t.slice(1).toLowerCase());
            } else {
              const raw = issue.description || "";
              const isProse =
                raw.length > 0 && !raw.trimStart().startsWith("<");
              if (isProse) {
                rec =
                  esc(
                    raw
          ...
          return `
      <tr data-filter-cat="${esc(issue.category)}">
        ...
        .join("")
    : '<tr><td colspan="5">No issue details collected.</td></tr>';
  ...
  const bestRowsHtml = bestRows.length
    ? bestRows
        .map(
          (row) => `
      <tr>
  ...
  const rowHtml = deviceRows
    .map(
      (row) => `
    <tr data-filter-health="${healthKey(scoreAverageForRow(row))}">
      <td><a href="${esc(row.url)}" target="_blank" rel="noopener noreferrer">${esc(row.url)}</a></td>
      <td>
  ...
  ${buildNextStepsHtml(deviceRows, commonIssues)}`;
}
...
function renderHtmlDashboard(summary, rows, generatedAt, sitemapUrl) {
  ...
  const { html: execSummaryHtml } = buildExecutiveSummary(
    rows,
    sitemapUrl,
    generatedAt,
  );
...
<head>
  ...
  <style>
    ...
    .heading { display: flex; flex-direction: column; gap: 16px; margin-bottom: 12px; }
    @media (min-width: 1024px) { .heading { flex-direction: row; justify-content: space-between; } }
    .meta { display: flex; flex-direction: column; gap: 4px; color: #555; margin-bottom: 20px; }
    ...
    table.fixed td:nth-child(1), table.fixed td:nth-child(2), table.fixed td:nth-child(3), table.fixed td:nth-child(7) { white-space: normal; word-break: break-word; overflow: visible; }
    ...
    .exec-summary { border-radius: 8px; padding: 20px 24px; margin-bottom: 28px; border-left: 5px solid #ccc; background: #f9f9f9; }
    .exec-summary.exec-pass { border-left-color: #0cce6b; background: #f2fcf6; }
    .exec-summary.exec-attention { border-left-color: #ffa400; background: #fffbf2; }
    .exec-summary.exec-critical { border-left-color: #ff4e42; background: #fff5f4; }
    .exec-summary.exec-unknown { border-left-color: #aaa; background: #f5f5f5; }
    .exec-header { display: flex; align-items: center; gap: 10px; margin-bottom: 10px; flex-wrap: wrap; }
    .exec-title { font-size: 16px; font-weight: 700; color: #222; }
    .exec-verdict { display: inline-block; font-size: 11px; font-weight: 700; padding: 3px 10px; border-radius: 20px; text-transform: uppercase; letter-spacing: .04em; color: #fff; }
    .exec-pass-badge { background: #0cce6b; }
    .exec-attention-badge { background: #ffa400; }
    .exec-critical-badge { background: #ff4e42; }
    .exec-unknown-badge { background: #888; }
    .exec-source { font-size: 11px; color: #999; margin-left: auto; font-style: italic; }
    .exec-body { margin: 0; line-height: 1.65; color: #333; font-size: 15px; }
    .next-steps { margin-top: 48px; padding: 24px 28px; background: #f0f4ff; border-left: 5px solid #3b82f6; border-radius: 8px; }
    .next-steps h2 { color: #1e3a5f; margin-bottom: 6px; }
    .next-steps-intro { color: #555; font-size: 14px; margin: 0 0 20px; }
    .next-steps-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 14px; }
    .next-step-item { display: flex; align-items: flex-start; gap: 14px; }
    .next-step-num { flex-shrink: 0; width: 24px; height: 24px; border-radius: 50%; background: #3b82f6; color: #fff; font-size: 13px; font-weight: 700; display: flex; align-items: center; justify-content: center; margin-top: -1px; }
    .next-step-text { font-size: 14px; line-height: 1.65; color: #333; }
    .next-step-score { font-size: 12px; color: #666; font-weight: 400; }
    .section-heading-wrap { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 0; }
    .section-heading-wrap h2 { margin: 0; }
    .filter-btn { background: none; border: 1px solid #d0d0d0; cursor: pointer; padding: 4px; border-radius: 4px; color: #888; display: inline-flex; align-items: center; flex-shrink: 0; }
    .filter-btn:hover { background: #f0f0f0; border-color: #b0b0b0; color: #444; }
    .filter-btn.active { color: #3b82f6; border-color: #3b82f6; background: #eff6ff; }
    .popover-wrap { position: relative; }
    .popover { position: absolute; right: 0; top: calc(100% + 6px); background: #fff; border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,.14); padding: 0; min-width: 230px; z-index: 200; display: none; }
    .popover.open { display: block; }
    .popover-section { padding: 14px 16px; }
    .popover-section + .popover-section { border-top: 1px solid #eee; }
    .popover-section-hdr { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
    .popover-section-title { font-weight: 600; font-size: 13px; }
    .popover-reset { background: none; border: none; color: #3b82f6; cursor: pointer; font-size: 12px; padding: 0; display: none; }
    .popover-reset.visible { display: inline; }
    .popover-options { display: flex; flex-direction: column; gap: 7px; }
    .popover-opt { display: flex; align-items: center; justify-content: space-between; gap: 6px; }
    .popover-opt label { display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: 13px; flex: 1; font-weight: 400; }
    .popover-opt input[type=checkbox] { cursor: pointer; width: 14px; height: 14px; flex-shrink: 0; }
    .popover-count { color: #999; font-size: 12px; min-width: 28px; text-align: right; }
  </style>
</head>
<body>
  <div class="heading">
    ...
    <div class="meta">
      ...
      <div><strong>Total pages scanned:</strong> ${rows.length}</div>
      <div><strong>Best performing pages:</strong> ${
        rows.filter((r) => {
          const s = scoreAverageForRow(r);
          return s !== null && s >= 80;
        }).length
      }</div>
      <div><strong>Worst performing pages:</strong> ${
        rows.filter((r) => {
          const s = scoreAverageForRow(r);
          return s === null || s < 80;
        }).length
  ...
  <script>
    ...
    // ---- Filters ----
    var tbodyOriginalRows = new Map();
    tableState.forEach(function(state) {
      if (state.tbody) tbodyOriginalRows.set(state.tbody, state.allRows.slice());
    });
    ...
    function applyFilters(tbody) {
      ...
      var state = tableState.get(table);
      if (!state) return;
      var origRows = tbodyOriginalRows.get(tbody);
      if (!origRows) return;
      var tbodyId = tbody.id;

      // Collect distinct filter keys for this tbody
      var allChecks = Array.from(document.querySelectorAll('.filter-check[data-table="' + tbodyId + '"]'));
      var filterKeys = [];
      allChecks.forEach(function(cb) {
        if (filterKeys.indexOf(cb.dataset.filterKey) === -1) filterKeys.push(cb.dataset.filterKey);
      });

      // Build active filters: key → checked values (only keys with ≥1 checked)
      var activeFilters = {};
      filterKeys.forEach(function(key) {
        var vals = Array.from(
          document.querySelectorAll('.filter-check[data-table="' + tbodyId + '"][data-filter-key="' + key + '"]:checked')
        ).map(function(cb) { return cb.value; });
        if (vals.length) activeFilters[key] = vals;
      });

      var hasFilters = Object.keys(activeFilters).length > 0;
      var filteredRows = hasFilters
        ? origRows.filter(function(tr) {
            return Object.keys(activeFilters).every(function(key) {
              return activeFilters[key].indexOf(tr.getAttribute('data-filter-' + key)) !== -1;
            });
          })
        : origRows.slice();

      // Hide excluded rows before renderPage so no stale rows remain visible
      var filteredSet = new Set(filteredRows);
      origRows.forEach(function(tr) {
        if (!filteredSet.has(tr)) tr.style.display = 'none';
      });

      state.allRows = filteredRows;
      state.resetPage();
      state.renderPage();

      // Update reset button visibility per section
      filterKeys.forEach(function(key) {
        var resetBtn = document.querySelector('.popover-reset[data-table="' + tbodyId + '"][data-filter-key="' + key + '"]');
        if (resetBtn) resetBtn.classList.toggle('visible', !!activeFilters[key]);
      });

      // Update filter button active state
      var sampleCheck = document.querySelector('.filter-check[data-table="' + tbodyId + '"]');
      if (sampleCheck) {
        var popoverEl = sampleCheck.closest('.popover');
        if (popoverEl) {
          var triggerBtn = document.querySelector('.filter-btn[data-popover="' + popoverEl.id + '"]');
          if (triggerBtn) triggerBtn.classList.toggle('active', hasFilters);
        }
      }
    }
    ...
    document.querySelectorAll('.filter-btn').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        var popoverId = btn.dataset.popover;
    ...
    document.querySelectorAll('.filter-check').forEach(function(cb) {
      cb.addEventListener('change', function() {
        var tbody = document.getElementById(cb.dataset.table);
        if (tbody) applyFilters(tbody);
      });
    });

    document.querySelectorAll('.popover-reset').forEach(function(btn) {
      btn.addEventListener('click', function(e) {
        e.stopPropagation();
        document.querySelectorAll('.filter-check[data-table="' + btn.dataset.table + '"][data-filter-key="' + btn.dataset.filterKey + '"]').forEach(function(cb) {
          cb.checked = false;
        });
        var tbody = document.getElementById(btn.dataset.table);
        if (tbody) applyFilters(tbody);
      });
    });
  </script>
