...
import { normalizeArray, average } from "./utils.js";
...
function renderTabPanel(device, rows) {
  ...
  const badges = [
    ...
    ["Avg Accessibility Findings", average(deviceRows.map((r) => r.accessibilityFindingCount))],
    ["Pages With Extended A11y Issues", deviceRows.filter((r) => r.pa11yIssueCount > 0).length],
  ]
    .map(
      ([label, value]) =>
        `<div class="card"><div class="label">${esc(label)}</div><div class="value">${value ?? "n/a"}</div></div>`,
    )
    .join("");
